...
class Article
{
 ...
 public static function getCountRows(  ) {
   ...
   public static function updateCountRows($value) {
   // $value = $value < 2 ? 2 : $value;
    $conn = new PDO( DB_DSN, DB_USERNAME, DB_PASSWORD );
    $sql = "UPDATE options SET value =:value WHERE optionName = :optionName";
    $st = $conn->prepare ( $sql );
    $st->bindValue( ":optionName", "countRows", PDO::PARAM_STR );
  ...
  */
  ...
  public static function getById( $id ) {
    $conn = new PDO( DB_DSN, DB_USERNAME, DB_PASSWORD );
    $sql = "SELECT id, title, summary, content, notes.userId, users.userName as userName FROM notes, users WHERE notes.userId = users.userId AND id = :id";//SELECT *  FROM notes WHERE id = :id
    $st = $conn->prepare( $sql );
  ...
  */
  ...
  public static function getList( $numRows=1000000, $order="title DESC" ) {
    $conn = new PDO( DB_DSN, DB_USERNAME, DB_PASSWORD );
    $sql = "SELECT id, title, summary, content, notes.userId, users.userName as userName FROM notes, users WHERE notes.userId = users.userId
            ORDER BY " . mysql_escape_string($order) . " LIMIT :numRows";//SQL_CALC_FOUND_ROWS * FROM notes
  ...
  */
  ...
  public function insert() {
    ...
    // Вставляем статью
    $conn = new PDO( DB_DSN, DB_USERNAME, DB_PASSWORD );
    $sql = "INSERT INTO notes (  title, summary, content, userId ) VALUES ( :title, :summary, :content, :userId )";
    $st = $conn->prepare ( $sql );
  ...
  */
  ...
  public function update() {
    ...
    // Обновляем статью
    $conn = new PDO( DB_DSN, DB_USERNAME, DB_PASSWORD );
    $sql = "UPDATE notes SET  title=:title, summary=:summary, content=:content WHERE id = :id";
    $st = $conn->prepare ( $sql );
  ...
  */
  ...
  public function delete() {
    ...
    // Удаляем статью
    $conn = new PDO( DB_DSN, DB_USERNAME, DB_PASSWORD );
    $st = $conn->prepare ( "DELETE FROM notes WHERE id = :id LIMIT 1" );
    $st->bindValue( ":id", $this->id, PDO::PARAM_INT );
    $st->execute();
    $conn = null;
  }
